...
class BookDataset(Dataset):
    def __init__(self, scalars, sequences, targets):
        self.scalars = torch.tensor(scalars).to(device)
        self.sequences = sequences
        self.sequences = [torch.tensor(x) for x in self.sequences]
        ...
        self.sequences = pad_sequence(self.sequences).transpose(0, 1).to(device)
